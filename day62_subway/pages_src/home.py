...
year_label = f'{year_min}년' if year_min == year_max else f'{year_min}~{year_max}년' # 조건연산자
...
with tab1:
    st.dataframe(
        summary_df.sort_values('날짜', ascending=False).head(30)[['날짜','역명','승하차','시간대컬럼','인원수','요일코드','주말여부']]
    )
with tab2:
