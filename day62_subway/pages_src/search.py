"""
폼 안에 입력 위젯들(역 이름, 승하구분, 기간, 시작시 범위, 요일) 조건들을 조작할 때마다
재실행되지 않고 "검색"버튼을 누르는 순간에만 한 번에 처리되도록 폼을 사용

만약 데이터가 많을 때 폼 없이 위젯을 조작하면 필터링을 다시 하면 체감 속도가
느려진다. --> st.form 사용
"""
...
with st.form('subway_search_form'):
    col1, col2 = st.columns(2) # 세로 2칸 1대1 비율
    with col1:
    ...
    date_range = st.date_input('조회 기간',
                               ...
                               max_value=df['날짜'].max())
    # 슬라이더 위젯
    hour_range = st.slider('조회시간대 (시작시 기준)',
                           min_value=5,
                           max_value=23,
                           value=(5, 23))

    # 멀티 설렉트 위젯
    weekday_pick = st.multiselect('요일',
...
if submitted:
    ...
    if keyword:
        filtered = filtered[filtered['역명'].str.contains(keyword)] # 일부만 입력해도 검색이되도록 하기
    ...
    # 슬라이더는 (최소값, 최댓값) 튜플을 반환 -> 시작시가 그 범위 안에 있는 행만 남김
    filtered = filtered[
    ...
    st.dataframe(
        filtered.sort_values('날짜', ascending=False).head(500)[['날짜','역명','승하차','시간대컬럼','인원수','요일코드','주말여부']]
    )
    if len(filtered) > 500:
        st.caption(f'표에는 상위 500건만 표시됩니다. \n전체 {len(filtered):,}건은 아래 버튼으로 다운로드 하세요')
