"""
즐겨찾는 역을 저장 --> 다른 위젯을 조작해도 사라지지 않게 한다.
...
# 'favorite_stations' 라는 키가 없을 때만 빈 리스트로 초기화
# 이 세션에서 처음 방문했을 때만
...
with col_btn:
    st.write('') # 버튼 높이를 selectbox와 맞추기 위한 여백용 빈줄
    st.write('')
    if st.button('+ 추가'):
...
if not st.session_state.favorite_stations:
    ...
    for i, station in enumerate(st.session_state.favorite_stations):
        ...
        with col_del:
            # key=f'del_{i}' 삭제 버튼이 여러개 일 때 서로 구분하기 위한 고유 key
...
if st.session_state.favorite_stations:
    # 즐겨찾기한 역이 여러개이면 그 중 하나를 골라 상세 패턴을 보게한다.
    focus_station = st.selectbox('조회할 역 선택', st.session_state.favorite_stations)
    ...
    # 문자열인 시간대컬럼으로 정렬하면 '10시~11시'가 '1시~2시'보다 먼저 정렬될 수 있다.
